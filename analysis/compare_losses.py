...
def compare_losses_across_models_with_sg_filter(output_dir, nof_losses, planar_losses, radial_losses):
    """
    ...
    nof_losses[:, 1] = savitzky_golay(nof_losses[:, 1], 381, 5)  # window size 51, polynomial order 3
    planar_losses[:, 1] = savitzky_golay(planar_losses[:, 1], 381, 5)  # window size 51, polynomial order 3
    radial_losses[:, 1] = savitzky_golay(radial_losses[:, 1], 381, 5)  # window size 51, polynomial order 3

    plt.plot(nof_losses[:, 0], nof_losses[:, 1])
    plt.plot(planar_losses[:, 0], planar_losses[:, 1])
    plt.plot(radial_losses[:, 0], radial_losses[:, 1])
    plt.title("Avg. Reconstruction Loss per Epoch")
    plt.ylabel("Avg. Reconstruction Loss")
    plt.xlabel("Epochs")
    legend_labels = ["STORN", "Planar", "Radial"]
    ...
    radial_losses[:, 2] = savitzky_golay(radial_losses[:, 2], 81, 3)  # window size 51, polynomial order 3

    plt.plot(nof_losses[:, 0], nof_losses[:, 2])
    plt.plot(planar_losses[:, 0], planar_losses[:, 2])
    plt.plot(radial_losses[:, 0], radial_losses[:, 2])
    ...
    nof_losses[:, 3] = savitzky_golay(nof_losses[:, 3], 381, 5)  # window size 51, polynomial order 3
    planar_losses[:, 3] = savitzky_golay(planar_losses[:, 3], 381, 5)  # window size 51, polynomial order 3
    radial_losses[:, 3] = savitzky_golay(radial_losses[:, 3], 381, 5)  # window size 51, polynomial order 3

    plt.plot(nof_losses[:, 0], nof_losses[:, 3])
    plt.plot(planar_losses[:, 0], planar_losses[:, 3])
    plt.plot(radial_losses[:, 0], radial_losses[:, 3])
...
if __name__ == '__main__':
    ...
    no_flow_logfile_path = "/home/abhishek/Dropbox/MAI/Internship-Thesis/TUM-Smagt/Thesis/" \
                           "results_worth_saving/raw_worthy_data/best_results/no_flow_2017_08_26_22_44_06/logfile.log"
    planar_logfile_path = "/home/abhishek/Dropbox/MAI/Internship-Thesis/TUM-Smagt/Thesis/results_worth_saving/" \
                          "flow_length_comparison/planar/2017_08_27_11_47_32/logfile.log"
    radial_logfile_path = "/home/abhishek/Dropbox/MAI/Internship-Thesis/TUM-Smagt/Thesis/results_worth_saving/" \
    ...
    output_dir = "/home/abhishek/Dropbox/MAI/Internship-Thesis/TUM-Smagt/Thesis/" \
                 "results_worth_saving/loss_comparison_plots/"  # Directory where the graphs will be saved.
    ...
    radial_losses = plots.helper_functions.read_loss_logfile(radial_logfile_path)
    print "radial_losses shape:", radial_losses.shape

    nof_losses = nof_losses[start_epoch:end_epoch, :]
    planar_losses = planar_losses[start_epoch:end_epoch, :]
    radial_losses = radial_losses[start_epoch:end_epoch, :]
    ...
    if remove_outlier:
        ...
        radial_losses = plots.helper_functions.remove_outliers(radial_losses, m=100)
        print "radial_losses sans outlier", radial_losses.shape
        compare_losses_across_models_with_sg_filter(output_dir, nof_losses, planar_losses, radial_losses)
    else:
        compare_losses_across_models_with_sg_filter(output_dir, nof_losses, planar_losses, radial_losses)
