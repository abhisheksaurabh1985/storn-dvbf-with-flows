...
def compare_losses_across_models_with_sg_filter(output_dir, rf_2_losses, rf_4_losses, rf_8_losses):
    """
    ...
    rf_2_losses[:, 1] = savitzky_golay(rf_2_losses[:, 1], 181, 3)  # window size 51, polynomial order 3
    rf_4_losses[:, 1] = savitzky_golay(rf_4_losses[:, 1], 181, 3)  # window size 51, polynomial order 3
    rf_8_losses[:, 1] = savitzky_golay(rf_8_losses[:, 1], 181, 3)  # window size 51, polynomial order 3

    plt.plot(rf_2_losses[:, 0], rf_2_losses[:, 1])
    plt.plot(rf_4_losses[:, 0], rf_4_losses[:, 1])
    plt.plot(rf_8_losses[:, 0], rf_8_losses[:, 1])
    ...
    rf_2_losses[:, 2] = savitzky_golay(rf_2_losses[:, 2], 181, 3)  # window size 51, polynomial order 3
    rf_4_losses[:, 2] = savitzky_golay(rf_4_losses[:, 2], 181, 3)  # window size 51, polynomial order 3
    rf_8_losses[:, 2] = savitzky_golay(rf_8_losses[:, 2], 181, 3)  # window size 51, polynomial order 3

    plt.plot(rf_2_losses[:, 0], rf_2_losses[:, 2])
    plt.plot(rf_4_losses[:, 0], rf_4_losses[:, 2])
    plt.plot(rf_8_losses[:, 0], rf_8_losses[:, 2])
    ...
    rf_2_losses[:, 3] = savitzky_golay(rf_2_losses[:, 3], 181, 3)  # window size 51, polynomial order 3
    rf_4_losses[:, 3] = savitzky_golay(rf_4_losses[:, 3], 181, 3)  # window size 51, polynomial order 3
    rf_8_losses[:, 3] = savitzky_golay(rf_8_losses[:, 3], 181, 3)  # window size 51, polynomial order 3

    plt.plot(rf_2_losses[:, 0], rf_2_losses[:, 3])
    plt.plot(rf_4_losses[:, 0], rf_4_losses[:, 3])
    plt.plot(rf_8_losses[:, 0], rf_8_losses[:, 3])
...
def compare_losses_across_models(output_dir, pf_2_losses, pf_4_losses, pf_8_losses):
    ...
    This one doesn't apply any smoothing. Needs to be changed to compare losses.
    :param output_dir:
    :param nof_losses:
    :param planar_losses:
    :return:
...
if __name__ == '__main__':
    ...
    output_dir = "/home/abhishek/Dropbox/MAI/Internship-Thesis/TUM-Smagt/Thesis/results_worth_saving/" \
                 "flow_length_comparison/radial_flow_length_plots/"
    ...
    rf_2_losses = rf_2_losses[start_epoch:end_epoch, :]
    rf_4_losses = rf_4_losses[start_epoch:end_epoch, :]
    rf_8_losses = rf_8_losses[start_epoch:end_epoch, :]
    ...
    if remove_outlier:
        ...
        rf_2_losses = plots.helper_functions.remove_outliers(rf_2_losses, m=1)
        print "rf_2_losses sans outlier", rf_2_losses.shape
        rf_4_losses = plots.helper_functions.remove_outliers(rf_4_losses, m=1)
        print "rf_4_losses sans outlier", rf_4_losses.shape
        rf_8_losses = plots.helper_functions.remove_outliers(rf_8_losses, m=1)
        print "rf_8_losses sans outlier", rf_8_losses.shape
        compare_losses_across_models_with_sg_filter(output_dir, rf_2_losses, rf_4_losses, rf_8_losses)
    else:
        compare_losses_across_models_with_sg_filter(output_dir, rf_2_losses, rf_4_losses, rf_8_losses)
