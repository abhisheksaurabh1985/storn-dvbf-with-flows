...
"""
Un comment the lines corresponding to CPF if it works. 
"""


# Output directory
output_dir = "/home/abhishek/Dropbox/MAI/Internship-Thesis/TUM-Smagt/Thesis/results_worth_saving/data_dist_plot/"

# Specify path to the pickled file
data_dir = "/home/abhishek/Dropbox/MAI/Internship-Thesis/TUM-Smagt/Thesis/results_worth_saving/data_dist_plot/"
...
plt.figure(0)
cos = np.vstack([cos_actual, cos_nf, cos_pf, cos_rf]).T
# cos = np.vstack([cos_actual, cos_nf, cos_pf, cos_cp, cos_rf]).T
bins_cos = np.linspace(-1, 1, 10)
plt.hist(cos, bins_cos, alpha=0.7, label=['Actual', 'NoFlow', 'Planar', 'Radial'])
...
plt.figure(1)
sin = np.vstack([sin_actual, sin_nf, sin_pf, sin_rf]).T
# sin = np.vstack([sin_actual, sin_nf, sin_pf, sin_cp, sin_rf]).T
bins_sin = np.linspace(-1, 1, 10)
plt.hist(sin, bins_sin, alpha=0.7, label=['Actual', 'NoFlow', 'Planar', 'Radial'])
...
plt.figure(2)
av = np.vstack([av_actual, av_nf, av_pf, av_rf]).T
# av = np.vstack([av_actual, av_nf, av_pf, av_cp, av_rf]).T
bins_av = np.linspace(-8, 8, 10)
plt.hist(av, bins_av, alpha=0.7, label=['Actual', 'NoFlow', 'Planar', 'Radial'])
...
plt.figure(3)
reward = np.vstack([reward_actual, reward_nf, reward_pf, reward_rf]).T
# reward = np.vstack([reward_actual, reward_nf, reward_pf, reward_cp, reward_rf]).T
bins_reward = np.linspace(-16, 0, 10)
plt.hist(reward, bins_reward, alpha=0.7, label=['Actual', 'NoFlow', 'Planar', 'Radial'])
